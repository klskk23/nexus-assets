...
- 015 起字段还可以绑到**型号**而不是类别，016 起还可以绑到**厂商**（该厂商旗下全部
  型号都会有它，包括以后新建的）。这类字段同样会作为该类别的一列出现，不属于那些型号的
  设备那一行留空——是**加列**不是改列，按列名取值不会因此报错。
  **注意它会自己长出来**：给一家厂商新加一款型号，或给一家厂商绑一个字段，
  这个类别的列集就变了；对面下次刷新会回 `needsConfirmation`，那是对的，
  nexus 不替人点头（见第五节）。
- `sys_vendor` 取的是厂商实体的名字（016 起 `product_models.vendor_id` 指向 `vendors`）。
  厂商改名后，**下次刷新数据源**这一列才会跟着变 —— 对面存的是刷新那一刻的行拷贝。
