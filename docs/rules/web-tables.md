...
**`font-heading` 给数值，不给标识符。** 统计数字、页码、流程序号是**读一眼的量**，
显示体让它们更醒目；**资产编号是标识符** —— 人对着实物标签和屏幕逐字符比对，
显示体只会让这件事更慢。能渲染不等于应该渲染，这正是 023 之后被推翻的那条理由。

所以：**资产编号一律普通字体 + `tabular-nums`**（列表页、概览、流转审计、详情页大标题都是）。
`tabular-nums` 不能省，同列的数字要对齐。
