...
| 内容列 | **按面板比例流动，地板 960，无上限** | `AppShell` 一处设定，见下 |
...
```
w-[min(100%,max(960px,76%))]
```

三段各司其职，**没有 `max-width`**：`76%` 是比例（留白恒占面板 24%）、
`max(960px, …)` 是地板（≤1619px 的窗口与 018 逐像素一致）、
`min(100%, …)` 是兜底（面板窄于 960 时贴合而不溢出，交给表格框自己横向滚动）。

**不要加上限。** 018 用的是固定 960，结果内容宽度恒定而留白随窗口无界生长 ——
实测占面板 14%（1440）→ 39%（1920）→ 56%（2560）→ 73%（3840）。
**天花板只是把同一个失效模式往外挪**：封在 1400，3840 下留白又回到 61%。

**不要给某一页单独设宽度。** 018 有过三档（960 / 760 / 620–640），020 全部拆掉。
判据：**它在决定「这一页多宽」还是「这个部件自己多宽」** ——
前者只有 `AppShell` 一处，后者（搜索框的 640、备注列的 `max-w-48`）留在原地。
`tests/contentColumn.test.ts` 守着这条：`routes/`、`features/` 下的 `max-w-`
命中集合必须等于白名单。

**比例要对着面板算，不是对着视口。** 侧栏与内边距是固定值，
按 `vw` 算会让留白占比从 14% 一路爬到 34%，那就不是「比例」了。
