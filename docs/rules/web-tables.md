...
**右内边距是 `pr-10`（40px），不是井槽的 12px。** 面板圆角 28px，内容再近就切进弧线，
右上角那两个按钮是第一个撞上去的。左 56 / 右 40 的差是有意的，再拉大就读成没对齐。
...
实测脚本 `specs/020-fluid-content-column/measure.mjs` 量五档窗口，
要求留白为 0、右缘恒 52px；把任何宽度规则贴回去它都会报错（已双向验证）。
