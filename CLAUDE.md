...
- **字段绑定有两种模式，互斥**（015 决策 96–100）。一个字段要么全绑类别
  ...
  **唯一性范围各算各的**：类别模式存类别 id（子树），型号模式存 `f:<field_id>`
  （该字段绑到的全部型号合起来）。按型号各自一个命名空间是错的——
  戴尔 ServiceTag 在整个产品线内唯一，不是每个型号内唯一。
  **`display_key` 只认类别模式字段**：型号字段只覆盖部分设备，其余的会永远退回 UUID 前 8 位。
