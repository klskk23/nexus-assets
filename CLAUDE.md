...
2. `specs/016-field-groups-vendors/`（决策 105–113）—— 本轮：字段组（绑定时展开的糖）、
   厂商成为实体、绑定目标变三个、互斥改为「类别 vs 设备」、型号实时继承厂商字段。
   上一轮 `specs/015-model-bound-fields/`（决策 96–104）：字段可以绑定型号
   （与绑类别互斥）。再上一轮 `specs/014-table-conventions/`（决策 89–95）：
...
- **有效字段集只有一个来源：`schema.EffectiveFields(categoryID)`**（015 决策 101）。
  它返回的是**这个类别的完整词汇表**（类别绑定 ∪ 该类别链上型号的绑定 ∪
  这些型号所属厂商的绑定），
