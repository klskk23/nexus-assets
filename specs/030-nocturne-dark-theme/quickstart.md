...
| 01 | 表单位置 | 靠左，装饰在右 | 居中；大圆探出左缘、小圆在右上、横线从右侧进来止于卡片外 50px、光晕在顶部正中 | 不改回：开发者 2026-09-19 看过靠右与两种居中后定了这版 |
| 02 | 三卡栅格 / 卡内距 | `minmax(300px,1fr)` gap 16 / 18 20 | 同 | 已改 |
| 02 | 内容宽 | 封顶 1280 | 铺满 | 不改回：开发者 2026-09-19 要求概览铺满页面 |
| 04 | 内容宽 | 封顶 1100 | 铺满 | 不改回：开发者 2026-09-19 要求设备详情铺满 |
| 03 | 选中行的粘住列 | 与行同色 | 曾是 `bg-well`（不透明地色），与行中间的主色 8% 不一致 | 已改：粘住格用不透明孪生色 `--row-selected` / `--row-hover` |
| 03 等 | 翻页条 | 区间左、每页 + 上下页右 | 同 | 已改（原先控件贴在区间旁边：`Pager` 在 `TableFrame` 底栏里没有撑满） |
