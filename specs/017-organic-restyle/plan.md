...
沿用既有技术栈。**唯一的依赖变动是移除 `next-themes`**（只有 `ui/sonner.tsx` 用到）。
...
| 迁移号 | **无**。本轮不碰数据库 |
| 新增资产 | 三款字体（Caprasimo / Figtree / Noto Sans SC 分片）+ 三份 OFL LICENSE |
...
| # | 原则 | 本特性的符合情况 |
|---|------|------------------|
| **I** 代码质量 | 分层边界不动：本轮不触及 `internal/`，唯一的服务端相关动作是**不动** `users.theme`。前端的分层保持现状（`components/ui` 是形状层，`features/` 是行为层，`routes/` 是页面）。**依赖净减一个**（移除 `next-themes`），不新增任何依赖 —— 字体是静态资产不是依赖。字体分片的生成脚本若需要新工具，走 `devDependencies` 且在任务里说明理由 |
...
| 风险 | 应对 |
|---|---|
| **中文字体分片后总体积超出 SC-009 的 8 MB** | 唯一预留的翻案点。实测产物后若超出：接受更大二进制并修订 SC-009，或退回「只自托管拉丁、中文用系统栈」并修订 FR-006。**修订规格，不放宽验收** |
