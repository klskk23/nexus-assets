...
- [ ] T027 [P] [US2] `web/tests/assets.test.tsx` 加测试：点表头勾选框全选本页，
      表格下方出现「选中符合当前筛选的全部 N 条」横幅（FR-012/FR-013）
- [ ] T028 [P] [US2] 同上加测试：Shift 连选一段、再 Shift 取消一段（FR-014）
- [ ] T029 [P] [US2] 同上加测试：改变筛选后**选中集不丢**，批量条计数如实反映（FR-015/SC-005）
- [ ] T030 [P] [US2] 同上加测试：点行尾按钮**不触发进入详情**（FR-017）
- [ ] T031 [P] [US2] 同上加测试：选中数为零时**没有**批量条；大于零时出现（FR-018）
- [ ] T032 [P] [US2] 同上加测试：筛选无命中时显示空态且能一键清除筛选
- [ ] T033 [US2] `web/src/features/assets/useSelection.ts` 新建：选中集以**资产 id 为键**
      （不是行下标），提供全选本页、扩选全部、Shift 连选（锚点是可见行序号）。
      筛选与翻页变化时选中集不变
- [ ] T034 [US2] `web/src/features/assets/SelectAllBanner.tsx` 新建：全选本页后出现的横幅，
      含「选中符合当前筛选的全部 N 条」与「按住 Shift 可连选」提示。**N 取列表接口已返回的 `total`**
- [ ] T035 [US2] `web/src/features/assets/BulkBar.tsx` 新建：底部常驻批量条（已选 N 台 +
      清空 + 各项批量动作）。仅在 N>0 时出现；表格容器留出底部内边距，**不遮住最后一行**（FR-019）
- [ ] T036 [US2] `web/src/routes/Assets.tsx`：接上以上三者；行尾 hover 出打印标签 /
...
- [ ] T038 [P] [US3] **找开发者确认「统计卡」与「类别分布条」两个自定义组件**
      （章程原则三：不存在的组件必须先确认，不接受事后补批）
- [ ] T039 [P] [US3] `web/tests/overview.test.tsx` 加测试：统计卡数字与对应筛选下的列表条数一致；
      分布条的填充比例与台数占比相符；「待我处理」每条可跳转
- [ ] T040 [US3] `web/src/features/overview/StatCard.tsx` 新建（确认后）
- [ ] T041 [US3] `web/src/features/overview/DistributionBar.tsx` 新建（确认后）。
      **填充块必须 `display: block`**，否则百分比宽度不生效；数值右对齐且 `tabular-nums`
- [ ] T042 [US3] `web/src/routes/Overview.tsx`：按稿子重排（标题行 → 四张统计卡 →
      左分布右待办的两列）。图表仍走 `lazy` + `Suspense`，**不得把 recharts 拉回入口 chunk**

## 第 6 阶段：US4 八个元数据页（P2）

- [ ] T043 [US4] `web/src/features/metadata/CrudPage.tsx`：Organic 形态。
      **改这一个，八个页面一起变** —— 字段、字段组、型号、厂商、状态、持有方、账号、角色
- [ ] T044 [US4] 跑八个页面的既有测试，确认**点击行编辑、右键出菜单、对话框内显示拒绝**
      三条行为一字未变（FR-021/FR-023/FR-026）
- [ ] T045 [US4] 逐页截图，**给开发者确认形态**（plan.md 的裁决权约定：
      稿子没画这四个页面，形态由实现者推导后交开发者判定）

## 第 7 阶段：US5 类别页与审计页（P3）

- [ ] T046 [P] [US5] `web/src/routes/Categories.tsx`：视觉重排。
      **不翻页、搜索时从树切平铺、清空恢复成树** —— 行为一字不变（FR-022）
- [ ] T047 [P] [US5] `web/src/routes/Audit.tsx`：视觉重排；前后值用绿/橙底标出新旧。
      **且区分不得只靠颜色** —— 加删除线/下划线之类的形状差异（FR-005）
- [ ] T048 [US5] `web/tests/audit.test.tsx` 加测试：新旧值的区分在不依赖颜色时仍然成立
- [ ] T049 [P] [US5] `web/src/routes/Import.tsx`：视觉跟上（流程页，不改流程）

## 第 8 阶段：收口

- [ ] T050 两份 i18n 全量复核：新增文案两种语言齐全；`tests/i18n.test.ts` 的孤儿检查为零
- [ ] T051 **英文界面逐页走查**：十二个页面在英文下没有文字溢出、
      药丸里没有换行、统计卡数字没有挤出（SC-007）
- [ ] T052 `docs/rules/web-forms.md` 与 `web-tables.md` 更新：药丸形态、圆角两档、
      深色已下架、字体自托管 —— 让下一个人不必重新发现
- [ ] T053 `CLAUDE.md` 复核：深色相关的表述清理干净
- [ ] T054 跑完整门禁（章程七条）：`gofmt -l` 空、`go vet`、`golangci-lint` 零告警；
...
- [ ] T055 按 [quickstart.md](./quickstart.md) 实机走完 17 步。
...
**第 5、6、7 阶段互不依赖**，可并行，也可只做一条就验收。

**T038 是一道闸**：两个自定义组件必须在动手前得到确认（章程原则三）。
第 5 阶段的其余任务都堵在它后面。
