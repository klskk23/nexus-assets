...
- [x] T001 造走查数据的脚本，写到 `specs/028-master-detail-holders/quickstart.md` 已描述的形状：
...
- [x] T002 `internal/asset/overview.go`：新增 `SubtreeCountsByHolder(ctx) (map[string]int, error)`。
...
- [x] T003 `internal/asset/query.go`：`ListFilter` 增加 `IncludeHolderDescendants bool`；
      `plainFilters` 里的持有方分支改为按开关走两条路 —— 为假走现有的
      `holder_type = ? AND holder_id = ?`，为真走同一棵递归 CTE 的子查询。
- [x] T004 [P] `internal/httpapi/handlers_holders.go`：新增 `GET /holders/counts` 处理器；
      `internal/httpapi/routes.go` 挂上。读默认全开放，与 `/categories/counts` 一致。
- [x] T005 [P] `internal/httpapi/handlers_assets.go`：读 `holder_include_descendants`，
      **默认 false**（与类别的 `include_descendants` 默认 true 故意不同，注释写明理由）。
- [x] T006 读同一套筛选的另外三处（`handlers_importexport.go`、`handlers_rows.go`、
...
- [x] T007 `internal/asset/overview_test.go`：`TestHolderSubtreeCounts` —— 三层树，
      含一个 0 台的节点，断言根的数字是子树合计而非它自己直接持有的。
- [x] T008 `internal/asset/overview_test.go`：`TestHolderCountsIncludeRetired` ——
      同一台报废设备，持有方计数**含它**、类别计数**不含它**。
      **红→绿双向验**：把状态过滤加回持有方那条，这条必须红。
- [x] T009 `internal/httpapi/holders_counts_test.go`：
...
- [x] T010 [P] `internal/httpapi/holders_counts_test.go`：
      `TestAssetsHolderFilterDefaultsToThatHolderAlone` —— 不带参数时行为与今天一致
      （既有链接不能被静默改变含义）。
- [x] T011 [P] 合约：`internal/httpapi/docs/openapi.yaml` 加 `/holders/counts` 与
...
- [x] T012 [P] [US1] `web/src/lib/types.ts`：`HolderEntity` 无需改动，确认即可；
      若缺 `is_default_stock` 之类的字段则补齐类型。
- [x] T013 [US1] `web/src/features/holders/holderRows.ts`：`flattenHolders`（深度优先、
      带 `isFolded`）、`searchHolders`（平展 + 沿 `parent_id` 爬出完整路径，**带界**）、
      `rootIDOf`（沿 `parent_id` 爬到根，**带界**；类别那份读物化路径，这份不能照抄）。
      **根排序按名字，类型不参与**（FR-004）。
- [x] T014 [US1] `web/src/features/holders/HolderTree.tsx`：`Rail` + `RailRow` + `TreePager`
      + `useFoldable` + `rootPaging`。行上带类型标识与设备数；底部「新建持有方」按钮
      （无 `holder.create` 时禁用 + 悬停说明）。**零新组件**。
- [x] T015 [US1] `web/src/features/holders/HolderDetail.tsx`：属性带（类型 / 上级**不可点** /
      默认库存点）+ **备注单独一块** + 「看这里的 N 台设备」链接 + 「修改」按钮。
      US3 的按钮在 T022 加进来。
- [x] T016 [US1] `web/src/features/holders/HolderEditor.tsx`：把 `Holders.tsx` 里的
      `EditDialog` 与删除、`RefusalAlert` 搬进来，**合并成一条拒绝路径**（两个 refusal 状态
      变一个，因为删除与保存都在同一个对话框里了）。
- [x] T017 [US1] `web/src/routes/Holders.tsx` 重写：`MasterDetail` + `useMasterSelection`
      （**传全部持有方的 id，不是本页的**）+ `PageHeader`。不再 import `CrudPage`。
- [x] T018 [US1] `web/src/routes/router.tsx`：加 `holders/:id`，与 `holders` 同一组件。
- [x] T019 [US1] i18n 两种语言：新增 plan 里列出的十一条；**删除**
      `holders.filterType` / `holders.filterStock` 两个孤儿。

### US1 的测试

- [x] T020 [US1] `web/tests/holdersTree.test.tsx`：层级正确（公司与无上级的位置并排在根）、
...
- [x] T020a [US1] `web/tests/holdersTree.test.tsx`：**左栏的分页与折叠**（FR-005、FR-006）。
...
- [x] T021 [US1] 改写 `web/tests/holderHierarchy.test.tsx`：表格断言换成树与对话框断言。
...
- [x] T022 [US2] `web/src/routes/Holders.tsx`：接上 `/holders/counts` 查询，
      喂给 `HolderTree` 的行尾与 `HolderDetail` 的链接。**同一个 map，两处读**——
      不是「两个各自算」（024 那条规矩的形状）。
- [x] T023 [US2] `HolderDetail` 的链接带 `holder_id=…&holder_include_descendants=true`。
- [x] T024 [US2] `web/src/routes/Assets.tsx`：确认从链接进来时 `holder_include_descendants`
      被读进筛选态并回传给服务端；不读的话数字与行数就对不上（026 的教训：
      **前端没跟上契约，而 mock 编码了旧契约所以测试全绿**）。

### US2 的测试

- [x] T025 [US2] `web/tests/holdersCountAgreement.test.tsx`：**行尾的数 == 链接文案里的数**，
...
- [x] T026 [P] [US2] `web/tests/holdersCountAgreement.test.tsx`：0 也写出来，不留白。
- [x] T027 [US2] 走查（quickstart 第 1、2 节）：根/中间层/叶子各验一次三者相等；
...
- [x] T033 [US4] `web/src/routes/Holders.tsx`：**删掉那个 `Promise.all` 的 usage 预取**。
- [x] T034 [US4] `HolderEditor.tsx`：打开删除确认时才查 `/holders/{id}/usage`，
      用它填确认框的描述（有多少条流转历史提到它）。
- [x] T035 [US4] 服务端拒绝时的 blockers 列在同一个对话框里（`RefusalAlert` 只剩一处落点）。

### US4 的测试

- [x] T036 [US4] `web/tests/holdersTree.test.tsx` 或独立文件：打开页面时**不发出**
      逐行的 usage 请求（断言请求列表，不是断言渲染）；点删除时**才**发出一次。
- [x] T037 [P] [US4] 拒绝时阻挡项显示在对话框内；确认框在删除前说明流转历史条数。
...
- [x] T038 [US5] `web/tests/categories.test.tsx`：补**根分页**一条 ——
      造多于一页的根，断言翻页后**任何一行的父都在同一页上**（014 决策 91 的守卫）。
- [x] T039 [US5] `web/src/features/categories/CategoryTree.tsx`：改掉顶部注释里
      「No folding, and no chevrons to fold with」那段 —— 它下面 30 行就在渲染 chevron。
      写成 025 的条件式折叠理由。
- [x] T040 [US5] `docs/rules/web-tables.md`：改写 024 决策 152 那段
      （「**不折叠。**…`CollapsibleTree` 当初就是这么被删的」），写明**为什么反过来**：
      不是原来判错了，是当初缺的那块零件（`rootPaging` 的按根分页、
      `useFoldable` 的条件式折叠 + 行上写明数量）后来做出来了。
- [x] T041 [P] [US5] 同文件：同段里「未指定类别时 `<Navigate replace>` 到第一个根类别」
...
- [x] T042 `docs/rules/domain.md`：写入**两种「有多少台」**——
      类别问「有多少台能用的」，持有方问「这里放着多少台」；
      **两个数不得在同一屏上顶着同一个词出现**（FR-015）。
- [x] T043 `docs/rules/web-tables.md`：写入持有方页的形状（第四个主从页）、
      右栏那处「只读看板」的例外及理由、以及**两处 SQL 靠一条比对测试钉住**这件事。
- [x] T044 [P] `web/src/features/metadata/CrudPage.tsx` 的使用者清单是否需要更新；
      `masterDetail.test.tsx` 里「调用方就是这三页」的守卫改成四页。
- [x] T045 **FR-035 检查**：确认 `MasterDetail` / `useMasterSelection` 本轮零改动。
      若被迫改了，**在交付说明里报告是什么迫使它改变**，不要顺手改掉了事。
- [x] T046 门禁全跑：`gofmt -l internal cmd`、`go vet ./...`、`go test ./...`、
...
- [x] T047 实机走查：按 `quickstart.md` 十节全走一遍，**含第 9 节**
...
- **`Assets.tsx` 把 `holder_include_descendants` 从地址里抹掉了**（T024 要防的正是它）。
  右栏说「看这里的 3 台设备」，点进去 **0 行** —— 024 花一整轮堵的那个坑，
  在一个新参数上重新裂开。DOM 测试当时没抓到，因为我只断言了链接的 href，
  没断言**列表页收到之后做了什么**。已修，并补了红→绿验过的守卫。
- **`RailRow` 的对齐缺陷（既有，四个 rail 都有）**：有折叠三角的行比同层没有三角的行
  右移 24px。此前没人看见，是因为另外三页的根一级要么都有子项要么都没有；
  持有方页的根一级同时有公司（有子项）和独立仓库（没有），一眼就是两列参差。
  修成「没有三角的行也把那 24px 留出来」，四页同时受益。

## FR-035 结论

**骨架零改动。** `git diff` 对 `MasterDetail.tsx` 与 `useMasterSelection.ts` 为空，
第四个调用方接入没有迫使它们改变。`RailRow` 改了一行 —— 它是形状件不是骨架，
且改的是一个既有缺陷，不是为持有方开的口子。
