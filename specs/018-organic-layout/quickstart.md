...
起一个本地实例，按顺序走完。这一轮**测试守不住版面**（35 个测试文件里断言
尺寸类的有 0 处），所以这份走查不是补充验证，**它就是主要的验收手段**。
...
**1. 内容列贴左且有上限。**
在 1920px 宽的窗口打开资产列表。内容列**不顶到右边**，宽度 ≤960px，
左侧紧贴主区内边距，**右侧是留白**。
再看类别页（正文类 760px）与流转表单（620–640px）。
**内容一路顶到右边即不通过。**（FR-001 / SC-001）
...
**4. 表格行高 48px。**
资产列表随便量一行 `<td>` 的高度 —— 48px（来自 15px 上下内边距）。
表头 `<th>` 的字重 600、色 `--secondary-foreground`。
**行高仍是 36px 即不通过。**（FR-004 / SC-001）
