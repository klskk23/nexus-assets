...
- [X] T061 [US7] ✅ **开发者裁定 2026-09-08：拆掉**（连同两份目录里的条目）。域名限制提示（017 加的，v1 唯一的准入边界）
      稿子的 `SignIn` 没有这一行。保留还是拆？**逐条确认项之一**
- [X] T062 [US7] ⚠️ **前提有误**：登录测试里**从来没有**这条断言 —— 任务写的时候没查。
      拆掉提示后 370 条全绿，孤儿检查按预期抓到了两份目录里的残留条目。原文：
      （或按 T056 的裁定更新）
...
- [X] T068 [P] [US9] `web/src/routes/Import.tsx`：按收敛后的语汇重排流程页
- [X] T069 [P] [US9] `web/src/routes/Audit.tsx`：重排。
      **前后值的新旧区分仍不得只依赖颜色**（017 的左侧竖线 + 标签，FR-025）
- [X] T070 [P] [US9] `web/src/routes/AssetHistory.tsx`：重排，时间线沿用 T044 的形态
- [X] T071 [US9] ✅ **开发者 2026-09-08 走查**：审计与全量历史通过；**导入页退回重做** ——
      「不应该直接在页面上分成 1 和 2，应该做成一个流程，卡片之类的」。已改成
      三张分步卡 + 圆点序号，序号从文案里搬出来。原任务：三页截图交开发者确认（FR-025）—— 它们没有稿子，
      是推导出来的，必须过一次人眼
