...
- [ ] T005 `web/src/components/ui/table.tsx`：`TableCell` 的 `px-2 py-1.5` 改成
      `px-5 py-[15px]`，字号 14px。**行高 48px 由内边距得出，不要给 `TableRow` 设 `h-12`** ——
...
- [ ] T022 [US2] 静态检查：`grep -rn "bg-card\|<Card" web/src --include='*.tsx'` 的命中集合
      必须等于白名单（`ActionBar`、`ui/card.tsx`，以及 T019 裁定后的 `ui/alert.tsx`）。
...
- [ ] T024 [US4] `web/src/features/metadata/CrudPage.tsx`：外层 `grid gap-6` → 分区 56px、
      区内成组 22–24px。**改这一个，八个元数据页一起变**（FR-024）
- [ ] T025 [US4] `web/src/routes/Assets.tsx`：内容列 960px 上限并贴左；
      **选择模型一件不动** —— 勾选列、全选框、横幅、Shift 连选、行尾三按钮、批量条
- [ ] T026 [US4] `web/src/routes/Categories.tsx`：节奏调到 56/24。
      **缩进树、搜索切平铺、名称格里没有行内箭头 —— 三条一字不改**（FR-023）
- [ ] T027 [US4] 跑八个元数据页的既有测试，确认**点击行编辑、右键出菜单、
      对话框内显示拒绝**三条行为一字未变
- [ ] T028 [US4] 跑资产页的 7 条选择模型回归测试（017 留下的），确认全绿（SC-005）
- [ ] T029 [US4] 截图：八个元数据页逐页拍，确认形态**完全一致** ——
      不一致说明有人绕过了共用件
...
- [ ] T030 [US1] `web/src/routes/AppShell.tsx`：主区加内容列上限（常规 960px），
      **贴左，右侧留白**。注意 research 第一节：现状不是「居中」而是「无界」，
      `max-w-7xl` 早就不在了 —— 要加的是上限本身
- [ ] T031 [P] [US1] 正文类页面（`Categories.tsx` 的说明区、`Import.tsx` 的流程说明）
      内容列 760px 上限
- [ ] T032 [P] [US1] 表单类（`features/transfers/TransferForm.tsx`）列宽 620–640px 贴左
- [ ] T033 [US1] 圆角收敛到三档（FR-006）：容器 28px、**下沉块与提示块 20px**、
      小控件 999px。20px 这一档是新的 —— 017 的 `--radius-md` 是 16px，
      现有的井与提示块都还在用它
- [ ] T034 [US1] 全站扫一遍 28px 圆角容器的内边距，**低于 24px 的补齐**（FR-003）——
      半径 28 的角会切到内容
- [ ] T035 [US1] 截图：在 1920px 宽的窗口拍资产列表与概览，
      确认内容不顶到右边、右侧是留白
...
- [ ] T036 [US3] `web/src/routes/AssetDetail.tsx`：h1 是资产编号（拉丁数字），
      **应当加上 `font-heading`** + `tabular-nums`。T007 摘掉的是中文标题上的，
      这里是反过来 —— **漏了这条就白改**（plan 风险表）
- [ ] T037 [P] [US3] 区标题统一 21px/700；计数、字段键名、页码加 `font-heading`
      （它们全是拉丁与数字）
- [ ] T038 [US3] 截图：一个中文页 + 资产详情，确认中文靠字号分层、拉丁走 Caprasimo

## 第 7 阶段：AppShell 两版 —— 开发者裁定（闸）

**这一阶段堵住后面所有页面的最终收敛。** 侧栏的语汇会决定其余页面怎么理解「浮起」。

- [ ] T039 A 版（稿子交付的）：`web/src/routes/AppShell.tsx` 侧栏 236px、
      `padding:36px 18px 48px 30px`、右边框 `1px solid var(--border-muted)`、
      裸数字编号带 `opacity:.6`、当前项整条满填陶土药丸、底部保留说明文字
- [ ] T040 B 版（稿子自己列的四条改进）：去掉右边框，侧栏做成一块**圆角 `--well` 浮在地面上**；
      编号做成 24px 圆（闲置描边、当前实心）；当前态只让圆点吃色 + 文字加重 +
      `--accent` 浅底；删掉底部说明文字
- [ ] T041 **两版各拍一张截图交开发者裁定。**
      提醒一句：改进①正是全局规则第 2 条本身 —— A 版会让外壳成为整轮里
      唯一一处用 hairline 分隔的地方
- [ ] T042 按裁定结果保留一版，删掉另一版，把理由写进 `docs/rules/web-forms.md`

## 第 8 阶段：US5 概览（P2）

- [ ] T043 [US5] `web/src/routes/Overview.tsx`：五个状态区块横向 flex-wrap，
      每个最小 152px，上芯片下 Caprasimo 34px 计数；**计数为 0 时数字退到次要色，
      但区块仍然可点**（筛出零条也是有效结果）
- [ ] T044 [US5] `web/src/routes/Overview.tsx`：类别分布 + 快速录入改成
      `1.55fr / 1fr`、间距 40px
- [ ] T045 [US5] `web/src/features/overview/DistributionBar.tsx`：轨道高 18px、
      类别名列 104px、计数列 46px 右对齐 Caprasimo。**整行可点**并跳到按类别筛选的列表
- [ ] T046 [US5] **找开发者确认**：分布条现在按**最大类别**取比例（017 的决定），
      稿子没说比例基准。保留还是改成按总数？**逐条确认项之二**
- [ ] T047 [US5] `web/src/features/transfers/Timeline.tsx`：行内边距 18px、
      鼠尾草圆点 10px、行间 `border-top: --border-muted`（末行补 bottom）
- [ ] T048 [US5] **找开发者确认**：时间线的「当前」标记（环形圆点 + 文字标，017 加的）
      稿子没画。保留还是按稿子拆掉？**逐条确认项之三**
- [ ] T049 [US5] `web/tests/overview.test.tsx`：补一条断言 ——
      状态区块计数为 0 时仍可点击（版面改动不得把它变成死块）
- [ ] T050 [US5] 截图概览页，对照 `Overview.dc.html`

## 第 9 阶段：US6 详情与流转（P2）

- [ ] T051 [US6] `web/src/routes/AssetDetail.tsx`：属性区改 `repeat(4, minmax(0,1fr))`
      网格，窄屏降 `repeat(2, ...)`，整块坐 `--well`、圆角 28、内边距 ≥24
- [ ] T052 [US6] `web/src/routes/AssetDetail.tsx`：页头 = 资产编号 h1 + 状态芯片
      （配合 T033 的 `font-heading`）
- [ ] T053 [US6] `web/src/features/transfers/TransferForm.tsx`：字段一律
      `Field`/`FieldGroup`，控件高 48px、圆角 999px
- [ ] T054 [US6] `web/tests/assetDetail.test.tsx`：补一条断言 ——
      属性网格渲染出全部字段（四列布局不得吞掉任何一个）
- [ ] T055 [US6] 截图详情与流转，对照 `AssetDetail.dc.html` 与 `Custody.dc.html`

## 第 10 阶段：US7 登录（P2）

- [ ] T056 [US7] `web/src/routes/Login.tsx`：`minmax(0,420px) / minmax(0,1fr)`、
      间距 64px、垂直居中、`min-height:560px`
- [ ] T057 [US7] `web/src/routes/Login.tsx`：产品名 Caprasimo 52px/1.05 两行；
      控件高 50px；「或」分隔线；整宽次级按钮
- [ ] T058 [US7] `web/src/routes/Login.tsx`：右侧四个绝对定位柔形
      （`--card` 300 / `--accent-2` 190 opacity .62 / `--primary` 132 opacity .9 /
      `--border` 描边空心 84），`aria-hidden`，**不含任何状态色槽**
- [ ] T059 [US7] **找开发者确认**：域名限制提示（017 加的，v1 唯一的准入边界）
      稿子的 `SignIn` 没有这一行。保留还是拆？**逐条确认项之一**
- [ ] T060 [US7] `web/tests/` 登录相关：确认域名提示在提交前可见的断言仍然成立
      （或按 T056 的裁定更新）
- [ ] T061 [US7] 截图登录页，对照 `SignIn.dc.html`

## 第 11 阶段：US8 打印对话框（P3）

- [ ] T062 [US8] `web/src/features/print/PrintDialog.tsx`：620px 上限、`--card` 底
      （**这一处用卡片是对的** —— 它真的浮起）、圆角 28、内边距 32、阴影
- [ ] T063 [US8] `web/src/features/print/PrintDialog.tsx`：提示块 `--well` 底、圆角 20、
      `padding:18px 22px`、左侧 Lucide info 图标 `stroke-width:2.75`
- [ ] T064 [US8] 作业列表：每行一张 `--well` 圆角 28 卡；药丸进度轨用**鼠尾草**
      （它数张数不数状态）；**只有「失败」那行用 `.status-red`**，因为那确实是一个状态
- [ ] T065 [US8] 截图打印对话框，对照 `PrintLabels.dc.html`
...
- [ ] T066 [P] [US9] `web/src/routes/Import.tsx`：按收敛后的语汇重排流程页
- [ ] T067 [P] [US9] `web/src/routes/Audit.tsx`：重排。
      **前后值的新旧区分仍不得只依赖颜色**（017 的左侧竖线 + 标签，FR-025）
- [ ] T068 [P] [US9] `web/src/routes/AssetHistory.tsx`：重排，时间线沿用 T044 的形态
- [ ] T069 [US9] **三页截图交开发者确认**（FR-025）—— 它们没有稿子，
      是推导出来的，必须过一次人眼

## 第 13 阶段：收口

- [ ] T070 静态检查两条：① `bg-card` / `<Card` 的命中集合等于白名单（T020）；
      ② `font-heading` 的命中里没有中文标题
- [ ] T071 两份 i18n 复核。**本轮预期零新增文案**；若有（如侧栏底部文字的替代），
      两种语言齐全，`tests/i18n.test.ts` 的孤儿检查为零
- [ ] T072 **英文界面逐页走查**：18 个路由在英文下药丸不换行、48px 行高不被撑破、
      固定列宽没把内容挤没（SC-007）
- [ ] T073 **键盘走查**：从页头 Tab 到页尾，每个可交互元素都到得了，
      焦点环（outline 2px + offset 2）始终可见（SC-008）
- [ ] T074 `docs/rules/web-forms.md` 更新：三档地面的判据、圆角三档、
      内容列上限、48px 行高来自内边距、焦点环是 outline 不是 ring —— 
      让下一个人不必重新发现
- [ ] T075 空状态复核（FR-030）：48px 行高与新的地面分工不适用于空态。
      逐个确认空状态仍说清什么会填满它，被筛空时仍给出清除筛选
- [ ] T076 跑完整门禁（章程七条）：`gofmt -l` 空、`go vet`、`golangci-lint` 零告警；
      `go test ./...` 全过；`nexus verify`；`npx tsc --noEmit`、`eslint`、
      `vitest run --maxWorkers=4`、`npm run build`；真镜像 `deploy/smoke.sh`
- [ ] T077 **验证 SC-009**：`git diff main --stat -- internal/ cmd/ migrations/` 必须为**空**。
      非空说明这一轮越界了
- [ ] T078 按 [quickstart.md](./quickstart.md) 实机走完 19 步，带尺子量
- [ ] T079 **前后对比**：把 T002 的基线截图与终版并排给开发者。
      「版面改好了没有」只有这样才回答得了
...
第 2 阶段 共用件（T004–T013）── 十个表格页 + 八个元数据页自动到位
        ↓
   ┌────┴────────────────────────────┐
第 3 阶段 US2 地面（T014–T023）   第 4 阶段 US4 列表（T024–T029）
   └────┬────────────────────────────┘
        ↓
第 5 阶段 US1 空间（T030–T035）· 第 6 阶段 US3 字体（T036–T038）
        ↓
第 7 阶段 【AppShell 两版 —— 开发者裁定】T039–T042
        ↓
   ┌────┼────────┬────────┐
  US5   US6     US7      US8      （第 8–11 阶段，互不依赖）
   └────┴────────┴────────┘
        ↓
第 12 阶段 US9 三个没画的页面（T066–T069）
        ↓
第 13 阶段 收口（T070–T079）
```
...
**六个需要开发者裁定的点**：T021（Alert 的 bg-card）、T041（侧栏两版裁定）、
T046（分布条比例基准）、T048（时间线当前态）、T059（登录域名提示）、
T069（三页确认）。**按阶段攒批问，不逐个打断** —— 开发者明确要求过。

**MVP = 第 1–4 阶段**（T001–T029）：共用件 + 地面 + 十八页里十个表格页到位。
到这里版面已经是新的了 —— 但按决策 129，本轮**一个分支做完再发**，不中途发布。

**任务总数 79，其中 12 个标了 `[P]`，6 个是需要开发者裁定的确认点。**
