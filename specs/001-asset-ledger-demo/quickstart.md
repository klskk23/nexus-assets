...
3. `/fields` 建信息项：
   - `mac` —— 静态键，MAC 地址类型，勾选「全局唯一」
   - `firmware` —— 静态键，文本
   - `sn` —— **表达式键**，勾选「全局唯一」，模板 `{{ .attrs.mac | hex2dec }}`
4. `/categories` 建「网络设备」→ 子类「SDWAN 路由器」，然后**按依赖顺序绑定**：
   先绑 `mac` 并勾必填，再绑 `firmware`，最后绑 `sn`。
   先绑 `sn` 会被拒绝，错误里会写明还缺哪个静态键、哪个需要改成必填。
5. 在子类的「显示编号」里选 `sn`。只有标为唯一的信息项会出现在这个下拉里。
6. `/models` 建一个型号，可以给它填 `attr_defaults`
7. 从首页的快速录入卡片选类别，录入第一台设备；录入页可以选型号，
   型号的默认值会填进尚未填写的字段
