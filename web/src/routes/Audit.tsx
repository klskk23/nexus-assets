  ...
  return (
    <div className="grid gap-5">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">{tAudit.hint}</p>
      </div>
      ...
      >
        <>
          <p className="text-sm text-muted-foreground">{tAudit.total(query.data?.total ?? 0)}</p>
              ...
              <TableBody>
                {(query.data?.items ?? []).map((e) => (
                  <TableRow key={e.id}>
                    ...
                    <TableCell>{tAudit.targets[e.target_type] ?? e.target_type}</TableCell>
                    <TableCell>
