...
export const zhMeta = {
  fields: {
    ...
    type: "类型",
    unique: "唯一",
    uniqueScopeHint: "范围随绑定方式：绑类别时在该类别子树内不重复，绑型号时在所绑的全部型号内不重复。",
    uniqueFixed: "唯一性在建好之后不能改：要打开它得先确认存量的值没有重复，并为每台设备补上记录。",
    keyFixed: "键名建好之后不能改：存量的值都记在这个键下。",
    typeFixed: "类型建好之后不能改：存量的值是按这个类型写进去的。",
    requiredScopeHint: "必填属于字段本身：勾上之后，它绑到的每个类别或型号都要求填。存量设备不回溯校验，下次编辑那台设备时才会要求补上。",
    ...
    allCategories: "全部类别",
    unbound: "未绑定",
    unboundHint: "还没有绑定到任何类别或型号。字段只有绑定后才会出现在录入表单里。",
    empty: "还没有任何字段",
